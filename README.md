...
.   You should observe loop packets being emitted on a regular basis. Once finished press Control-C to exit.

**Note:** You will only see loop packets and not archive records when running the driver directly. This is because you are seeing the direct output from the driver and not WeeWX. 
...
2.  If installing the driver using the *wee_extension* utility (the recommended method) install the GW1000 driver using the *wee_extension* utility as per *Installation as a WeeWX driver* step 2 above.
...
.   You should observe loop packets being emitted on a regular basis. Some or all loop packets should include GW1000 data. Once finished press Control-C to exit.

**Note:** When the driver file is run directly with the *--test-service* a series of small loop packets are emitted every 10 seconds to simulate a running WeeWX instance. The GW1000 is polled and the GW1000 data added to the loop packets when available. As the default GW1000 poll interval is 60 seconds it is likely that not all loop packets will be augmented with GW1000 data.
